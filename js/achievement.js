function getStyle(element, attr) {
    ...
    window.onload=function(){
        var ach_select3=document.getElementById('ach_select3').getElementsByTagName('li');
        var ach_select_content=document.getElementById('ach_select_content');

        ach_select3[0].onmouseover=function(){
            ach_select3[1].className='';
            this.className='ach_active';
            startMove(ach_select_content,'left',0);

        }

        ach_select3[1].onmouseover=function(){
            ach_select3[0].className='';
            this.className='ach_active';
            startMove(ach_select_content,'left',-310);

        }

        //下面轮播效果
        var item_content=document.getElementById('item_img_content');
        var a=-5;
        var k = 0;
        setInterval(function(){
            if (parseInt(k)<-1190) {
                item_content.style['left']='0px';
                k=0;
            }
            else{
                var iCurrent = item_content.offsetLeft;
                k=item_content.style['left']=iCurrent+a+'px';
            }
        },30);
